...
CREATE TYPE USERFLAIRS AS ENUM ('none', 'Verified', 'Distinguished', 'Eminent');
...
CREATE TABLE IF NOT EXISTS USERS (
    ...
    USER_NICK TEXT DEFAULT 'Nony Mouse',
    USER_THUMB TEXT,
    USER_FLAIR Text REFERENCES USERFLAIRS(FLAIR) DEFAULT 'none',
    USER_ROLE Text REFERENCES USERROLES(ROLE) DEFAULT 'user',
    USER_LEVEL Text REFERENCES USERLEVELS(LEVEL) DEFAULT 'wood',
    USER_STARS INT DEFAULT 1,
    IS_DELETED BOOL DEFAULT FALSE,
    HAS_BASIC_AUTH BOOL DEFAULT TRUE,
...
ALTER TABLE USERS ADD CONSTRAINT USERS_NICK_LENGTH CHECK (length(USER_NICK) BETWEEN 3 and 64);
ALTER TABLE USERS ADD CONSTRAINT USERS_THUMB_LENGTH CHECK (length(USER_THUMB) BETWEEN 12 and 128);
...
ALTER TABLE SESSIONS ADD CONSTRAINT SESSIONS_ID_LENGTH CHECK (length(UNQID) = 128);
